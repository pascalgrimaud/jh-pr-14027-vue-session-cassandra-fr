...
/**
 * Performance test for the CassTestEntity entity.
 */
class CassTestEntityGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CassTestEntity entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cassTestEntities")
            .get("/api/cass-test-entities")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cassTestEntity")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cassTestEntity_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cassTestEntity")
                .get("${new_cassTestEntity_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cassTestEntity")
            .delete("${new_cassTestEntity_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
