...
/**
 * Performance test for the CassTestMapstructEntity entity.
 */
class CassTestMapstructEntityGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CassTestMapstructEntity entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cassTestMapstructEntities")
            .get("/api/cass-test-mapstruct-entities")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cassTestMapstructEntity")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cassTestMapstructEntity_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cassTestMapstructEntity")
                .get("${new_cassTestMapstructEntity_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cassTestMapstructEntity")
            .delete("${new_cassTestMapstructEntity_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
